	...
	foreach ( $taxonomies as $taxonomy ) {
		...
		if ( ! empty( $core_taxonomies ) ) {
			$fields[] = array(
				array(
					'value'   => $taxonomy->name,
					'checked' => isset( $post_type['taxonomies'] ) && in_array( $taxonomy->name, $post_type['taxonomies'] ) ? $taxonomy->name : '',
					'key'     => $taxonomy->name,
					'name'    => $taxonomy->name,
					'id'      => $taxonomy->name,
...
function npt_get_post_types( $taxonomies ) {
	...
	foreach ( $post_types as $post ) {
		...
		$fields[]         = array(
			array(
				'value'   => $post->name,
				'checked' => isset( $taxonomies['post_types'] ) && in_array( $post->name, $taxonomies['post_types'] ) ? $post->name : '',
				'key'     => $post->name,
				'name'    => $post->name,
				'id'      => $post->name,
...
function npt_post_type_custom_column_content( $column, $post_id ) {
	...
	switch ( $column ) {
		case 'menu-label' :
			...
			if ( $post_type ) {
				echo '<span class="list-table-icon">' . get_npt_svg_icon( $post_type['menu_icon'], 18, '#50575e' ) . ' ' . $post_type['labels']['menu_name']
				     . '</span>';
			} else {
				_e( 'Unable to get slug', 'npt' );
			}
			break;
		case 'slug' :
			$post_type = get_post_meta( $post_id, 'npt_post_type', true );
			...
			if ( $post_type ) {
				...
				if ( $post_type['taxonomies'] ) {
					foreach ( $post_type['taxonomies'] as $taxonomy ) {
						...
						echo '<span style="margin-right: 15px;">  ' . $tax_label . ' </span>';
					}
				}
				foreach ( $taxonomies as $taxonomy ) {
					$npt_taxonomy_id      = get_page_by_path( $taxonomy->name, OBJECT, 'npt-taxonomy' );
					$selected_tax = get_post( $npt_taxonomy_id );
					echo '<a style="margin-right: 15px;" href="' . get_edit_post_link( $npt_taxonomy_id ) . '"> ' . $selected_tax->post_title . ' </a>';
				}
			}
			...
			if ( $taxonomies ) {
				...
				foreach ( $post_type as $post ) {
					if ( $post == 'page' || $post == 'post' || $post == 'attachment' ) {
						...
						echo '<span style="margin-right: 15px;">  ' . $post_type_label . ' </span>';
					} else {
						$npt_post_type_id            = get_page_by_path( $post, OBJECT, 'npt-post-type' );
						$selected_post_type = get_post( $npt_post_type_id );
						echo '<a style="margin-right: 15px;" href="' . get_edit_post_link( $npt_post_type_id ) . '"> ' . $selected_post_type->post_title . ' </a>';
					}
				}
			} else {
				_e( '<span aria-hidden="true">—</span>', 'npt' );
